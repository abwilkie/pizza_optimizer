...
class ToppingsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create topping" do
    assert_difference('Topping.count') do
      post toppings_url, params: { topping: { available: @topping.available, name: @topping.name } }
    end
  ...
  test "should update topping" do
    patch topping_url(@topping), params: { topping: { available: @topping.available, name: @topping.name } }
